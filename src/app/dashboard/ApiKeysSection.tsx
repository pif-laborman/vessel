...
export function ApiKeysSection() {
  ...
  return (
    <div>
      {/* Revealed key banner */}
      {revealedKey && (
        <div className="card" style={{ marginBottom: 'var(--space-4)', background: '#D1FAE5', border: '1px solid #6EE7B7' }}>
              ...
              <tbody>
                {keys.map((k) => (
                  <tr key={k.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '10px 16px', color: 'var(--text-primary)' }}>{k.name}</td>
      ...
      {showForm && (
        <div className="card" style={{ marginTop: 'var(--space-3)', display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
          ...
          <button
            ...
            disabled={creating}
            className="btn-primary"
            style={{ fontSize: 'var(--text-sm)', padding: '6px 16px', opacity: creating ? 0.6 : 1 }}
          >
